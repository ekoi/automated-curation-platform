...
class DataverseIngester(Bridge):
    ...
    def execute(self) -> BridgeOutputDataModel:
        ...
        if self.target.input:
            ...
            md_json.update({self.target.input.from_target_name:
        ...
        for _ in db_manager.find_non_generated_files(dataset_id=self.dataset_id):
            ...
            # logger(f'{self.__class__.__name__} f_json: {f_json}', settings.LOG_LEVEL, self.app_name)
        ...
        if self.target.target_url_params:
            self.target.target_url = (self.target.target_url + "?" + self.target.target_url_params.
                                      replace("$PID", md_json["datasetVersion"]["datasetPersistentId"]))
        ...
        if dv_response.status_code == 201:
            ...
            if self.target.metadata and self.target.metadata.transformed_metadata:
                ...
                if ingest_file.get("status") == status.HTTP_200_OK:
                    ...
                    if self.target.initial_release_version and self.target.initial_release_version == ReleaseVersion.PUBLISHED:
                        logger(f'Publish the dataset', settings.LOG_LEVEL, self.app_name)
                        publish_status = self.__publish_dataset(pid)
                        message = "The dataset is successfully published" if publish_status == status.HTTP_200_OK \
                            else "The dataset is unsuccessfully published"
                else:
                    ingest_status, message = DepositStatus.ERROR, ingest_file.get("message")
                    delete_response = requests.delete(f"{self.target.base_url}/api/datasets/{dv_id}/versions/:draft",
        ...
        for file in db_manager.find_non_generated_files(dataset_id=self.dataset_id):
            ...
            if jsonData:
                ...
                if file.mime_type == "application/zip":
                    ...
                    logger(f'Start zipping file {file.name}. Real path: {zip_file_name}', settings.LOG_LEVEL
                           , self.app_name)
                    zip_a_zipfile_with_progress(zip_file_name, file.path)
                    os.remove(zip_file_name)
                    logger(f'Finished zipping file {file.name} to {real_file_path} in '
                           f'{round(time.perf_counter() - start, 2)} seconds', settings.LOG_LEVEL, self.app_name)
                ...
                logger(f'Finish ingesting file {file.name} to {pid} in {round(time.perf_counter() - start, 2)}'
                       f' seconds.',settings.LOG_LEVEL, self.app_name)
